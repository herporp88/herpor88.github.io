...
omdb_api_key <- readLines("~/Downloads/SDS 264/SDS264_S26/omdb_api_key.txt")
print(omdb_api_key)
```
...
```{r}
#| include: FALSE
#It reads your API key from a text file and stores it in a variable called omdb_api_key
# omdb_api_key <- readLines("~/R/SDS264/Inclass_264/OMDB_KEY.txt") #However, the file does not exist. 

# Alternatively, load your OMDB API key using:
omdb_api_key <- Sys.getenv("OMDB_KEY")

# Find url exploring examples at omdbapi.com
url <- str_c("http://www.omdbapi.com/?t=coco&apikey=", omdb_api_key)
...
movies <- c("Rambo", "Avatar", "Titanic", "Inception", "Barbie")
...
for(i in 1:5) {
  ...
  omdb[i,5] <- as.numeric(details$Metascore)
  omdb[i,6] <- as.numeric(details$imdbRating)
  omdb[i,7] <- as.numeric(str_remove_all(details$BoxOffice, "[$,]"))
}
...
### STEPS 
#1. Request an API key: [here](https://www.nps.gov/subjects/developer/get-started.htm)

#2. After you get your key, save your key in .Renviron using: usethis::edit_r_environ("project")

#3. After you saved your key, you can load your key using: Sys.getenv()

4. then load your dataset

```{r}

usethis::edit_r_environ("project")  # opens an .Renviron window

# Add a line like: NPS_KEY='myspecialkey'
# Save the .Renviron file
# Close down RStudio
# Restart RStudio
...
np_kaggle <- read_csv("~/Downloads/SDS 264/SDS264_S26/parks.csv")
...
# park_code[1] get the first

# Try grabbing elements for one park
url1 <- str_c("https://developer.nps.gov/api/v1/parks?parkCode=" #how did you get this? 
...
# Alternatively, load your NPS API key using:
census_api_key <- Sys.getenv("My_key")

url1 <- str_c("https://api.census.gov/data/2020/acs/acs5?get=NAME,B01003_001E,B19013_001E&for=tract:*&in=state:27&in=county:053", "&key=", census_api_key)
acs5 <- GET(url1)
...
#Use the map function to run the loop for each year. 
rice_tracts_2019_2021 <- map(years, 
~ MN_tract_data(year = .x, county = "Rice",
                variables = c("B01003_001",
                              "B19013_001")) |>
                               mutate(year = .x)) |>
  list_rbind() #stacks all those data frames on top of each other to create a dataset


#After building the dataset
#First, clean the variables by changing the variable names 
rice_tracts_2019_2021 <- rice_tracts_2019_2021 |>
  rename(
...
#Second, find the income over time by calculating the mean for each year. 
rice_income_trend <- rice_tracts_2019_2021 |>
  group_by(year) |>
  summarize(avg_median_income = mean(median_income, na.rm = TRUE))

#create a ggplot for the mean income. 
ggplot(rice_income_trend, aes(x = year, y = avg_median_income)) +
  geom_line() +
  geom_point() +
  labs(title = "Rice County Census Tract Median Income Over Time",
    x = "Year",
    y = "Average Median Household Income")

#Third, find the total population over time by calculating the sum for each year. 
rice_population_trend <- rice_tracts_2019_2021 |>
  group_by(year) |>
  summarize(total_population = sum(population, na.rm = TRUE))

#Create a ggplot for these three years
ggplot(rice_population_trend, aes(x = year, y = total_population)) +
  geom_line() +
  geom_point() +
  labs(title = "Rice County Population Over Time",
    x = "Year",
    y = "Total Population")
```
